<script lang="ts">
  ...
  export let handleConfirm = async () => {
    if (formData.cohort && distItems.includes(formData.cohort)) {
      shake = true;
      setTimeout(() => {
        shake = false;
      }, 750);
...
>
  <div class="mt-3 pt-2 text-center sm:mt-0 sm:ml-4 sm:text-left">
    ...
    <div class="block h-full min-h-[6rem] w-full ">
      ...
      {#if formData.cohort && distItems.includes(formData.cohort)}
        <p class="text-sm text-red-500">Cohorts need to be unique.</p>
      {/if}
      <div class="flex hidden flex-row items-center">
        <Icon
