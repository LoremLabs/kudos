<script lang="ts">
  ...
  export let handleConfirm = async () => {
    ...
    if (formData.confirmDistList !== 'DELETE') {
      shake = true;
      setTimeout(() => {
        shake = false;
      }, 750);
  ...
  export let form = {
    ...
    inputs: [
      ...
      {
        name: 'confirmDistList',
        required: true,
        type: 'string',
        displayName: 'Delete? Type the "DELETE" to Confirm',
      },
      // {
      //   type: 'string',
      //   name: 'lastName',
  ...
  const handleKeydown = (e) => {
    ...
    if (e.key === 'Tab') {
      if (lastTab === 0) {
        ...
        if (Date.now() - lastTab < 500) {
          e.preventDefault();
          formData.confirmDistList = 'DELETE';
        }
        lastTab = 0;
      }
    }
      ...
      <div class="ml-4 flex">
        ...
        <div class="ml-3">
          ...
          <div class="mt-2 text-sm text-red-700">
            <p>
              This will delete the list and all of its contents. This action
              cannot be undone.
            </p>
          </div>
        </div>
