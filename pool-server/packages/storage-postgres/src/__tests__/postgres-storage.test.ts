...
import { createTestStorage } from "./helpers/setup.js";
import { PostgresStorage } from "../postgres-storage.js";
import type { Event } from "@kudos-protocol/core";
import { normalizeEvent } from "@kudos-protocol/core";
...
describe("schema & init", () => {
  ...
  it("migrations are idempotent", async () => {
    const s2 = await createTestStorage();
    await s2.close();
  });

  it("postgres storage is initialized", async () => {
...
describe("transaction atomicity", () => {
  it("rolls back events and projections together on failure", async () => {
    ...
    // Monkey-patch updateProjections to throw on the second call
...
describe("outbox", () => {
  let outboxStorage: PostgresStorage;

  beforeEach(async () => {
    outboxStorage = await createTestStorage({ outbox: true });
  });

  afterEach(async () => {
    await outboxStorage.close();
  });
  ...
  it("leasePending respects maxAttempts filter", async () => {
    ...
    for (let i = 0; i < 5; i++) {
      const rows = await outboxStorage.leasePending(
        100,
        10,
        `lease-${i}`,
        0,
      ...
      if (rows.length > 0) {
        await outboxStorage.markFailed(
          rows.map((r) => r.id),
          "test error",
          `lease-${i}`,
        );
    ...
    const rows = await outboxStorage.leasePending(100, 5, "final-lease", 0);
    expect(rows).toHaveLength(0);
  });
  ...
  it("leasePending sets lease_id and leased_at", async () => {
    ...
    expect(rows).toHaveLength(1);
    const rows2 = await outboxStorage.leasePending(
      100,
      5,
      "another-lease",
      60,
  ...
  it("markDelivered sets delivered=1", async () => {
    ...
    await outboxStorage.appendEvents("pool-1", [event]);
    const rows = await outboxStorage.leasePending(100, 5, "test-lease", 60);
    await outboxStorage.markDelivered(
      rows.map((r) => r.id),
      "test-lease",
    );

    const remaining = await outboxStorage.leasePending(
      100,
      5,
      "test-lease-2",
      0,
